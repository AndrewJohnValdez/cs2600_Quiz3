...
PtrToEmployee searchEmployeeByNumber(PtrToConstEmployee ptr, int tableSize, long targetNumber){ 
 ...
 { 
    //ptr -> number is going down every reference, searching for the targetNumber
    //-> will dereference the variable for number to then compare
...
PtrToEmployee searchEmployeeByName(PtrToConstEmployee ptr, int tableSize, char * targetName){ 
 ...
 for(; ptr < endPtr; ptr++) 
 { 
    if(strcmp(ptr->name,targetName) == 0) //strcmp = string compare
