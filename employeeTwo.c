...
EmployeeTwo is a alternative of searching the employee than following the methods of EmployeeOne
...
//static function means that it is only usable in a file
...
{
    return searchEmployeePhone(ptr, size, phone, compareEmployeePhone);
}

PtrToEmployee searchEmployeeBySalary(PtrToConstEmployee ptr, int size, double salary)
{
    return searchEmployeeBySalary(ptr, size, &salary, compareEmployeeSalary);
}